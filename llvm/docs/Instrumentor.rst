...
The **Instrumentor** is a highly configurable instrumentation pass for LLVM that allows users to insert custom runtime function calls at various program points. Unlike traditional instrumentation tools that are hardcoded for specific purposes (like sanitizers or profilers), the Instrumentor provides a flexible, configuration-driven approach where users can specify:
...
- **PRE**: Insert instrumentation **before** the operation occurs
  ...
  - For functions: instrument at function entry
...
- **Function filtering**: Exclude runtime functions from instrumentation (automatic)
